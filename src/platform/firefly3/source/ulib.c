...
int8_t v;
    ...
    if(uart_rx_buf_start==uart_rx_buf_end) {
       ...
       v=nrk_event_wait(uart_rx_signal); 
       if(v==NRK_ERROR)
          nrk_kprintf(PSTR("uart rx sig failed\r\n" ));
   }
...
#if 0 /* don't touch the pins, let the user configure them */
#ifdef SPI_SS_PULLUP
...
#endif
	DDRD  = BM(LED_0) | BM(LED_1) | BM(LED_2) | BM(LED_3); 
