...
if(pin==NRK_EXT_INT_3) { EIMSK |= BM(INT3); return NRK_OK; }
...
if(pin==NRK_EXT_INT_3)
	{
	ext_int3_callback=callback_func;
	if(mode==NRK_LOW_TRIGGER) EICRA &= ~(BM(ISC31) | BM(ISC30));
	if(mode==NRK_LEVEL_TRIGGER) 
		{ EICRA &= (~BM(ISC31)); EICRA |= BM(ISC30); }
	if(mode==NRK_FALLING_EDGE) 
		{ EICRA |= BM(ISC31); EICRA &= (~BM(ISC30)); }
	if(mode==NRK_RISING_EDGE) EICRA |= BM(ISC31) | BM(ISC30);
	return NRK_OK;
	}
...
SIGNAL(INT3_vect) {
	if(ext_int3_callback!=NULL) ext_int3_callback();
	else
	nrk_kernel_error_add(NRK_SEG_FAULT,0);
	return;  	
}
