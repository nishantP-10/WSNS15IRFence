...
#define NUM_LEDS 4 // number of LEDs on the board

// RSSI_STEP_DB = ceil((MAX_RSSI_VAL + 1) / 2^NUM_LEDS)
// max PHY_ED_LEVEL is 84 (datasheet p.51), so to cover all values we need
// a step of ceil(85/16) = 6 dB, but since we don't see above 65 in practice,
// it's safe to pick a step of 5 dB, which covers range [0..5*15=75] db
#define RSSI_STEP_DB 5
...
static void display_rssi(uint8_t rssi)
{
  uint8_t b; // LED indexes are in order
  rssi /= RSSI_STEP_DB; // bin the rssi value
